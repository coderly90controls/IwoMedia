 ...
 *******************************************************************************/
package com.android.iwo.util.view.refresh;
...
import com.android.iwo.util.view.refresh.internal.FlipLoadingLayout;
import com.android.iwo.util.view.refresh.internal.LoadingLayout;
import com.android.iwo.util.view.refresh.internal.RotateLoadingLayout;
import com.android.iwo.util.view.refresh.internal.Utils;
import com.android.iwo.util.view.refresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		// Styleables from XML
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PullRefresh);

		if (a.hasValue(R.styleable.PullRefresh_preMode)) {
			mMode = Mode.mapIntToValue(a.getInteger(R.styleable.PullRefresh_preMode, 0));
		}

		if (a.hasValue(R.styleable.PullRefresh_preAnimationStyle)) {
			mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(
					R.styleable.PullRefresh_preAnimationStyle, 0));
		}
		 ...
		 * Styleables from XML
		 */
		if (a.hasValue(R.styleable.PullRefresh_preRefreshableViewBackground)) {
			Drawable background = a.getDrawable(R.styleable.PullRefresh_preRefreshableViewBackground);
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		} else if (a.hasValue(R.styleable.PullRefresh_preAdapterViewBackground)) {
			Utils.warnDeprecation("ptrAdapterViewBackground", "ptrRefreshableViewBackground");
			Drawable background = a.getDrawable(R.styleable.PullRefresh_preAdapterViewBackground);
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		}

		if (a.hasValue(R.styleable.PullRefresh_preOverScroll)) {
			mOverScrollEnabled = a.getBoolean(R.styleable.PullRefresh_preOverScroll, true);
		}

		if (a.hasValue(R.styleable.PullRefresh_preScrollingWhileRefreshingEnabled)) {
			mScrollingWhileRefreshingEnabled = a.getBoolean(
					R.styleable.PullRefresh_preScrollingWhileRefreshingEnabled, false);
		}
